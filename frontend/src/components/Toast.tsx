...
const DISMISS_MS_WITH_LINK = 10000;
...
function ToastItem({
  ...
  useEffect(() => {
    const t = setTimeout(
      () => onRemove(toast.id),
      toast.txHash ? DISMISS_MS_WITH_LINK : DISMISS_MS,
    );
    return () => clearTimeout(t);
  }, [toast.id, toast.txHash, onRemove]);
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm text-gray-200 leading-relaxed">{toast.message}</p>
        {toast.txHash && (
          <a
