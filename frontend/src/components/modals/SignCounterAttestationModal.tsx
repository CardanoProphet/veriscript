...
export function SignCounterAttestationModal({
  wallet,
  attestation,
  protocolDatum,
  ...
  return (
    <Modal title="Sign Counter-Attestation" onClose={onClose}>
      <div className="space-y-4">
        {/* Attestation being countered */}
        <div className="rounded-xl bg-gray-800/60 border border-white/10 p-4 space-y-3">
          <h3 className="text-sm font-semibold text-white">You are disputing:</h3>
          ...
          <div className="flex items-center gap-2 pt-1">
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20">
              ...
              <span className="text-xs text-emerald-400 font-medium">
                {attestation.signerCount} endorser{attestation.signerCount !== 1 ? "s" : ""}
              </span>
            </div>
            {attestation.counterSignerCount > 0 && (
              <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-red-500/10 border border-red-500/20">
                <Icons.ShieldX className="w-3.5 h-3.5 text-red-400" />
                <span className="text-xs text-red-400 font-medium">
                  {attestation.counterSignerCount} counter{attestation.counterSignerCount !== 1 ? "s" : ""}
                </span>
              </div>
            )}
          </div>
        ...
        {/* Warning */}
        <div className="rounded-lg bg-red-400/10 border border-red-500/20 p-3 text-xs text-red-300 leading-relaxed">
          <strong>Important:</strong> By signing, you publicly state that you dispute this attestation —
          e.g. the source code does not compile to the stated script hash, or the source link is
          incorrect. This is a permanent on-chain counter-record.
        </div>

        {/* Signer selection */}
        <div>
          <p className="text-sm font-medium text-gray-300 mb-2">Counter-sign with</p>
          {loadingSigners ? (
